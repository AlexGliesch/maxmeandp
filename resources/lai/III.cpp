...
void Outputing(int tt) {
  ...
  sprintf(buff, "%s_%d_%d.txt", "III", N, tt);
  fp = fopen(buff, "a+");
...
int main(int argc, char** argv) {
  ...
  //  seed = 5000; //seed= 1000 for N = 1000, seed= 3000 for N =3000, seed= 5000 for N =
  //  5000 srand( seed ) ;

  int sign;
  N = 5000; // N = 1000, 3000, 6000.
  w = new double[N];
  D = new double*[N];
  for (i = 0; i < N; i++)
    D[i] = new double[N];
  for (tt = 1; tt <= 10; tt++) {
    ...
    for (i = 0; i < N; i++) {
      ...
      w[i] = 1.0 + (1.0 * sign * (rand() % 3000)) / 30000.0;
    }
